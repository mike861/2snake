...
# -*- coding: utf-8 -*-
"""
Network Communication Module - Provides multiplayer battle functionality for Snake Game
"""
...
logger.add("logs/network_{time}.log", level="DEBUG", rotation="1 day", retention="7 days")
...
class GameServer:
    ...
    def _process_message(self, message, conn):
        ...
        try:
            ...
            # Find the player's connection (sanity check)
...
def run_server(host=DEFAULT_HOST, port=DEFAULT_PORT):
    """Run the game server"""
    logger.warning(f"Starting server mode on {host}:{port}")
...
def run_client(host='localhost', port=DEFAULT_PORT):
    """Run the game client"""
    logger.warning(f"Starting client mode, connecting to {host}:{port}")
    client = GameClient(host=host, port=port)
    
    # Set callback functions
    def on_state_update(state):
        logger.warning(f"Game state updated: food position {state['food']}")
    
    def on_game_start(state, player_index):
        logger.warning(f"Game started, you are player {player_index+1}")
    
    def on_game_over(data):
        logger.warning(f"Game ended: {data}")
    ...
    if client.connect():
        logger.warning("Connected successfully, waiting for game to start...")
        
        try:
            while client.is_connected():
                time.sleep(1)
                
                # Send random direction occasionally
                if random.random() < 0.3 and client.game_id:
                    direction = random.choice([[1, 0], [-1, 0], [0, 1], [0, -1]])
                    logger.warning(f"Sending move: {direction}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Snake Game Network')
    parser.add_argument('--server', action='store_true', help='Run as server')
    parser.add_argument('--client', action='store_true', help='Run as client')
    parser.add_argument('--host', default='localhost', help='Host address (server to bind, client to connect)')
    parser.add_argument('--port', type=int, default=DEFAULT_PORT, help='Port number')
    
    args = parser.parse_args()
    
    if args.server:
        logger.warning("Starting server mode...")
        run_server(host=args.host, port=args.port)
    elif args.client:
        logger.warning("Starting client mode...")
        run_client(host=args.host, port=args.port)
    else:
        logger.warning("Please specify --server or --client parameter")
